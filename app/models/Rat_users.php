...
class Rat_Users extends Eloquent implements UserInterface, RemindableInterface {
    ...
    public static function initialize($uid) {
        $done = false;
        DB::transaction(function ($uid) use ($uid) {
            // Starting Traits values
            $traits =  Traits::getAll();
            Rat_User_Trait::initialize($uid, $traits);
            // Starting Skills values
            $skills = Rat_Skills::getAll();
            Rat_User_Skill::initialize($uid, $skills);
            // Money in current items and Total Items
            Rat_User_Item::initializeMoney($uid);
            Rat_User_Current_Item::initializeMoney($uid, Rat_User_Item::getMoney($uid)[0]->qty);
            $done = true;
        });
        return $done;
    }
